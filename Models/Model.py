...
class HARTrans(torch.nn.Module):
    def __init__(self, args):
        ...
        self.transformer = HARTransformer(270, args.hlayers, args.hheads, 500)
        self.args = args
        self.kernel_num = 128
        self.kernel_num_v = 16
        self.filter_sizes = [10, 40]
        self.filter_sizes_v = [2, 4]
        self.pos_encoding = Gaussian_Position(270, 350, args.K)

        if args.vlayers == 0:
            self.v_transformer = None
            self.dense = torch.nn.Linear(90, 5)
        else:
            self.v_transformer = Transformer(350, args.vlayers, args.vheads)
        ...
        for i, filter_size in enumerate(self.filter_sizes_v):
            ...
            self.__setattr__(enc_attr_name_v,
                             torch.nn.Conv1d(in_channels=350,
