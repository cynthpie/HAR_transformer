...
def get_args():
    ...
    parser.add_argument('--input_length', type=int, default=1000,
                        help='input length for avg pooling in raw data [default: 2000]')
    parser.add_argument('--output_model_dir', type=str, default='./1p_CLEAN_runs',
...
def load_data(root):
    data = torch.load("Data_clean_1p_w1000.pt")
...
def write_to_file(conf_matrix):
    f = open("conf_matrix_1p_clean.txt", mode='w+', encoding='utf-8')
...
def c_main():
    ...
    logfile_name = os.path.join(args.output_model_dir, "THAT_1p_clean_test08.txt") ## CHANGE ME
...
def cross_validation():
    ...
    logfile = open("1p_clean_cross_validation.txt", "w")
