...
def get_args():
    ...
    parser.add_argument('--hlayers', type=int, default=6,
                        help='horizontal transformer layers [default: 6]')
    parser.add_argument('--hheads', type=int, default=27,
    ...
    parser.add_argument('--vheads', type=int, default=100,
    ...
    parser.add_argument('--filter_size', type=list, default= [10, 35, 70, 100],
    ...
    parser.add_argument('--input_length', type=int, default=700,
                        help='window size for avg pooling in raw data [default: 2000]')
    parser.add_argument('--dropout_rate', type=int, default=0.5,
                        help='dropout_rate [default: 0.5]')
    parser.add_argument('--clip_gradient', type=int, default=1.0,
                        help='clip_gradient [default: 0.8]')
    parser.add_argument('--output_model_dir', type=str, default='./ALL_runs', ## CHANGE DEFAULT
...
def load_data(root):
    data = torch.load("/home/cynthia/transformer_har/Data_all_w700.pt")
...
def c_main():
    ...
    logfile_name = os.path.join(args.output_model_dir, "THAT_all_test012.txt")
...
def cross_validation():
    ...
    logfile = open("all_cross_validation_2.txt", "w")
    ...
    outfile = open(THAT_predictions, "w")
    for i in range(len(outputs)):
        outfile_name.write(f"True labels: {Y_train[i, :]}, predicted labels: {(outputs[i, :]>0.5).long()}\n")
    outfile.close
